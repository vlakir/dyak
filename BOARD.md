...
- **T027** — Фамилии-нарицательные (Бивень, Кузнец) по умолчанию не склоняем:
  склонение только при грамеме `Surn` у pymorphy; иначе именительный (как в
  офиц. документах). Обход — `decline_surnames` в конфиге; `check` показывает.
  ADR в DECISIONS [closed 2026-06-25, PR `T027-common-noun-surnames`].
