...
_SYNONYMS = {
    ...
    'фио': FULLNAME,
    'ф_и_о': FULLNAME,  # «Ф.И.О.» после нормализации спецсимволов (T026)
...
# Роли, на которых завязано СПЕЦ-склонение: их потеря при коллизии реально
# снижает функциональность, поэтому о ней предупреждаем. `position`/
# `personal_number` с T024 вестигиальны — любая колонка склоняется generic
# фраз-движком, а литералы (коды/даты) исключаются по форме значения, — поэтому
# их коллизия безвредна (обе колонки отработают верно) и не зашумляет вывод.
_GATING_ROLES = frozenset({SURNAME, NAME, PATRONYMIC, FULLNAME, RANK})
...
# Прогон НЕ-идентификаторных символов (пробелы И спецсимволы: /, №, -, . …):
# Jinja-тег `{{ заголовок }}` требует валидный идентификатор, поэтому такой
# прогон сжимается в одно `_` при нормализации заголовка («л/н» → «л_н»).
...
def normalize_header(raw: str) -> str:
    """
    Привести заголовок к валидному Jinja-идентификатору.

    Пробелы И спецсимволы (`/`, `№`, `-`, `.` и т.п.) ломают разбор тега
    `{{ заголовок }}`, поэтому любой их непрерывный прогон сжимается в одно
    подчёркивание, крайние снимаются: «Номер приказа» → «Номер_приказа»,
    «л/н» → «л_н», «Ф.И.О.» → «Ф_И_О». Авто-фиксер тегов
    (`render/engine.fix_jinja_spaces`) применяет ту же нормализацию к «голым»
    тегам, поэтому обе стороны (ключ контекста и тег шаблона) совпадают.
    """
    return _NON_IDENT.sub('_', raw.strip()).strip('_')
...
def is_literal_value(text: str) -> bool:
    """
    Значение — литерал (личный номер/код/дата), который НЕ склоняется (T024).

    В модели «склонение по умолчанию» любая текстовая ячейка оборачивается
    склоняемой фразой; литералы по ФОРМЕ значения исключаются из обёртки —
    так коды (`ХУ-123456`, `1`) и даты не зашумляют `check`/`reverse` ложным
    склонением, причём независимо от роли колонки (снимает T020-аналог для
    двух код-колонок: и победившей роль `personal_number`, и проигравшей).
    """
    stripped = text.strip()
    return bool(_PERSONAL_NUMBER_RE.match(stripped) or _DATE_RE.match(stripped))
...
def recognize(
    ...
    for header in headers:
        ...
        if role is None:
            continue
        if role in raw:
            if role in _GATING_ROLES:
                logger.warning(
                    'Колонка «%s» претендует на роль «%s», уже занятую колонкой '
                    '«%s» — игнорирую',
                    header,
                    role,
                    raw[role],
                )
            # Вестигиальная роль (position/personal_number) — коллизия безвредна
            # (склонение по умолчанию), не предупреждаем, оставляем первую.
            continue
        raw[role] = header
