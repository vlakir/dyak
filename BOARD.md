...
- **T032** — Доработка справки GUI перед 0.3.3: приём «КАПС → обычное
  написание» через `{{ … | падеж | title }}` (+ `examples/README` +
  регресс-тест) и раздел про автофильтр (генерация по видимым строкам); кода
  не добавлялось [closed 2026-06-26, PR `T032-case-title-help`].
