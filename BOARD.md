...
- **T001** — Этап 0 «Скелет»: пайплайн «таблица → документы» на простой
  ...
  `T001-skeleton`).
  Acceptance: `dyak generate --table employees.xlsx --template order.docx
  --out ./output [--config dyak.yaml] [--sheet ...]` создаёт по одному
  docx на строку; pydantic-конфиг с маппингом колонок (`dyak.yaml`);
  чтение xlsx через `openpyxl`; рендер через `docxtpl` (переменные
  подставляются строкой, склонения ещё нет); имя файла рендерится из
  `config.filename`; коллизии имён → суффикс `_2/_3` + предупреждение.
  Все 4 проверки зелёные, coverage ≥ 80%.
